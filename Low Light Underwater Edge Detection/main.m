% This code is taken directly from Tunai Marques on GitHub,
% URL: https://github.com/tunai/l2uwe
% This includes  code in /third party and /utils which are referenced at end of file
...
img_original = imread('./images/w20.jpg');
...
save_outputs = 0; % 1/0 = save/don't save outputs
outPath = '.\out\';
...
grey_og = im2gray(img_original)
grey_re = im2gray(result)

% Add Gaussian blur to reduce noise
blur = fspecial('Gaussian', 5, 5);
smooth_og = imfilter(grey_og, blur, 'replicate');
smooth_re = imfilter(grey_re, blur, 'replicate');

% Apply Canny edge detector
edge_og = edge(smooth_og, 'canny');
edge_re = edge(smooth_re, 'canny');

figure;imshow(edge_og);
figure;imshow(edge_re);

figure;subplot(2,2,1);imshow(img_original);title('Original Image');
subplot(2,2,2);imshow(result);title('Resulting Image after L^2UXE');
subplot(2,2,3);imshow(edge_og);title('Canny Edge Detection on Original');
subplot(2,2,4);imshow(edge_re);title('Canny Edge Detection on Result');
